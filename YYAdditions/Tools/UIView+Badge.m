...
//  YiyangDoctorNew
//
//  Created by eyoung on 2019/2/22.
...
- (void)updateBadgeFrame
{
    ...
    CGFloat badgeLeft = self.width-minWidth-padding+5;
    self.badge.frame = CGRectMake(badgeLeft, self.badgeOriginY, minWidth + padding, minHeight + padding);
...
//显示红点
- (void)showBadgePoint:(BOOL)show{
    if (show) {
        if (!self.badge) {
            self.badge                      = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 10, 10)];
            self.badge.centerX              = self.width-5;
            self.badge.centerY              = 5;
